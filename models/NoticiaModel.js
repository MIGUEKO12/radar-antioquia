...
// ================= SECCIÓN: INSERCIÓN =================
function insertarNoticia({ titulo, link, fecha, descripcion, subregion: _subregion, municipio: _municipio, categoria: _categoria, modo, query }) {
  ...
  const result = db.run(
    `INSERT INTO noticias (titulo, link, fecha, descripcion, subregion, municipio, categoria, modo, query, hash, score)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      titulo,                       // Título completo de la noticia
      link,                         // URL de origen
      fecha,                        // Fecha ISO ajustada a hora Colombia
      descripcion || null,          // Snippet del RSS (mejora re-detección futura)
      subregion  || 'general',      // Subregión detectada por el gazetteer
      municipio  || null,           // Slug canónico del municipio (sin tildes)
      categoria  || 'general',      // Categoría del clasificador + filtro
      modo       || 'antioquia',    // 'antioquia' (cron) o 'libre' (búsqueda manual)
      query      || null,           // Query RSS que originó la noticia
      hash,                         // MD5 del título para deduplicar
      score                         // Puntaje de confiabilidad de la fuente
    ]
  );
...
function reclasificarTodo() {
  ...
  // Se traen también descripcion, subregion y municipio actuales para poder
  // comparar y re-detectar con TODO el texto disponible (título + snippet RSS).
  const noticias = db.all('SELECT id, titulo, link, descripcion, categoria, subregion, municipio FROM noticias', []);
  let actualizadas = 0;
  let sinCambio    = 0;

  for (const n of noticias) {
    try {
      const categoriaBase = clasificarNoticia(n.titulo);                       // Clasificación base por título
      const categoriaNew  = aplicarFiltro(n.titulo, categoriaBase, n.link || ''); // Filtro de calidad/contexto
      const { subregion, municipio } = detectarUbicacion(n.titulo, n.descripcion || ''); // Detección v2

      // BUG CORREGIDO: antes solo se actualizaba si cambiaba la CATEGORÍA.
      // Ahora se actualiza si cambia CUALQUIERA de los tres campos.
      const cambioCategoria = categoriaNew !== n.categoria;                    // ¿Categoría distinta?
      const cambioUbicacion = (subregion || 'general') !== (n.subregion || 'general')
                           || (municipio || null)      !== (n.municipio || null); // ¿Ubicación distinta?

      if (cambioCategoria || cambioUbicacion) {
        db.run(
