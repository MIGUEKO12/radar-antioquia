...
async function fetchNoticias(query, modo = 'antioquia') {
  ...
  return items.map(item => {
    ...
    // ── Descripción del RSS: Google News la entrega como HTML con enlaces a
    //    coberturas relacionadas. Se eliminan las etiquetas y entidades HTML
    //    para quedarnos solo con el texto plano (títulos relacionados).
    //    Ese texto suele contener el municipio cuando el título principal no.
    const descripcionCruda = item.description?.[0] || '';           // HTML crudo del RSS
    const descripcion = descripcionCruda
      .replace(/<[^>]*>/g, ' ')                                     // Quitar etiquetas HTML
      .replace(/&nbsp;|&amp;|&quot;|&#\d+;|&[a-z]+;/g, ' ')         // Quitar entidades HTML
      .replace(/\s+/g, ' ')                                         // Colapsar espacios
      .trim()                                                       // Limpiar extremos
      .slice(0, 1000);                                              // Límite defensivo de tamaño
    ...
    // Detección de ubicación con título + descripción (v2 del gazetteer)
    const { subregion, municipio } = detectarUbicacion(titulo, descripcion);
    ...
    return { titulo, link, fecha, descripcion, subregion, municipio, categoria, modo, query };
  });
}
