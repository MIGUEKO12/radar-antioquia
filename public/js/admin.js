...
// 5 toques seguidos en la MARCA de la barra (la montañita "Antioquia")
// abre el login admin. Antes vivía en el logo "RA" del header, que fue
// retirado del diseño; el selector apunta ahora a la marca nueva.
let _logoClicks = 0;
let _logoTimer  = null;

document.addEventListener('DOMContentLoaded', () => {
  const logo = document.querySelector('.barra-marca'); // Marca actual (montaña + nombre)
  if (!logo) return;                                   // Guarda: si tampoco existe, no romper
  // SIN cursor:pointer a propósito: el disparador es un acceso SECRETO y el
  // cursor normal no debe delatar que la marca es interactiva (seguridad por
  // discreción — el visitante común nunca sabrá que ahí hay una puerta).
  logo.addEventListener('click', () => {
    _logoClicks++;                                     // Acumular toques consecutivos
    clearTimeout(_logoTimer);                          // Reiniciar la ventana de tiempo
    if (_logoClicks >= 5) {                            // Al 5.º toque...
      _logoClicks = 0;                                 // Reiniciar contador
      if (AdminState.activo) salirAdmin();             // Si ya es admin → salir
      else abrirLoginAdmin();                          // Si no → abrir login
    } else {
      _logoTimer = setTimeout(() => { _logoClicks = 0; }, 2000); // 2s para completar los 5
    }
  });
});
